  ...
  const reports = [
    {
      ...
      progress: '출고 완료',
      review: '관리자 입력용 샘플 데이터입니다. 실제 사례 등록 후 공개됩니다.',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16 max-w-3xl mx-auto">
          ...
          <p className="text-lg text-brand-body leading-relaxed">
            고객님의 조건에 맞춘 정확한 진단과 상담 사례를 투명하게 공개합니다.<br className="hidden md:block" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reports.map((report) => (
            <div key={report.id} className="bg-brand-card border border-white/10 flex flex-col clip-chamfer overflow-hidden group hover:border-brand-yellow/30 transition-colors">
              {/* Report Header (Status) */}
              <div className="bg-black/50 p-4 border-b border-white/5 flex justify-between items-center">
                <div className="text-xs font-mono font-bold text-brand-body/60">REPORT NO. {String(report.id).padStart(3, '0')}</div>
                <div className={`text-xs font-bold px-2 py-1 rounded ${report.progress === '출고 완료' ? 'bg-brand-green/20 text-brand-green' : 'bg-brand-yellow/20 text-brand-yellow'}`}>
                  {report.progress}
                </div>
              </div>
              ...
              <div className="p-6 flex-grow space-y-4">
                ...
                <div className="pt-2">
                  ...
                  <p className="text-sm text-brand-body/90 leading-relaxed bg-black/30 p-3 rounded-sm border-l-2 border-brand-yellow">{report.keyPoint}</p>
                </div>

                <div className="pt-2">
                  <div className="text-xs font-bold text-brand-green mb-1">추천 방향</div>
                  <p className="text-sm text-brand-body/90 leading-relaxed bg-black/30 p-3 rounded-sm border-l-2 border-brand-green">{report.recommendation}</p>
                </div>
              </div>

              {/* Sample Review & Image Notice */}
              <div className="mt-auto p-4 border-t border-white/5 bg-black/40 flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs text-brand-body/60 italic leading-relaxed">
                    {report.review}
                  </p>
                </div>
              </div>
        ...
        <div className="text-center mt-12">
          <Button variant="outline" size="lg" onClick={() => window.location.href = '#consult-form'}>
