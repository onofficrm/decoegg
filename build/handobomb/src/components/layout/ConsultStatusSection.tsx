...
export function ConsultStatusSection() {
  // 실제 데이터 연동 시 이 배열을 API 응답으로 대체합니다.
  const consultList = [
  ...
  // consultList = []; // 데이터가 없을 경우를 테스트하려면 이 주석을 해제하세요.

  const getStatusColor = (status: string) => {
    switch (status) {
      case '상담 접수': return 'text-white bg-white/10 border-white/20';
      case '조건 확인': return 'text-brand-yellow bg-brand-yellow/10 border-brand-yellow/30';
      case '서류 확인': return 'text-brand-orange bg-brand-orange/10 border-brand-orange/30';
      case '차량 추천': return 'text-brand-green bg-brand-green/10 border-brand-green/30';
      case '출고 준비': return 'text-[#00E5FF] bg-[#00E5FF]/10 border-[#00E5FF]/30';
      case '출고 완료': return 'text-brand-bg bg-white border-white';
      default: return 'text-brand-body bg-white/5 border-white/10';
    }
  };

  return (
    <section className="py-24 bg-brand-bg border-t border-white/5 relative">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-brand-body">
            고객님의 상황에 맞춘 1:1 맞춤 상담이 실시간으로 진행되고 있습니다.
          </p>
        </div>
